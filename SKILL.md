...
| 17 | Gán prompt trực tiếp qua DOM `.textContent` / `.innerHTML` vào Slate Editor | Trạng thái React nội bộ của Slate không nhận diện, nút Tạo vẫn bị disable/mờ | BẮT BUỘC focus ô editor trước, sau đó lấy React `editor` từ `__reactProps*`, gán `editor.selection = null`, gọi `editor.insertText(promptText)` và `editor.onChange()`. Không dùng các lệnh DOM thô hoặc execCommand thiếu sync |
| Click context menu / Tải xuống 1080p bị trượt, lặp đi lặp lại hoặc timeout | Khi click 1080p/4K lần đầu, web Flow gửi request mạng `batchAsyncGenerateVideoUpsampleVideo` (trigger upscale ngầm) chứ không có file tải về ngay. Giải pháp: Lắng nghe song song sự kiện download và request mạng upsample bằng `Promise.race` hoặc check sự xuất hiện của file mới trong thư mục (đối chiếu `manifest.json` UUID `editId` thay vì chỉ quét regex tên file cứng). | Right-click phải dùng coordinate center thực tế hoặc Playwright native click `{ button: 'right', force: true }` để bypass hoàn toàn các button Radix UI che khuất. Bắt buộc gọi reset `scrollTop = 0` (cuộn lên đỉnh) trước khi click từng video trong worker loop để tránh grid virtualized unmount các video mới ở đầu grid. Thiết lập `page.setViewportSize({ width: 1920, height: 1080 })` ngay sau connect CDP để đồng bộ viewport ảo với màn hình thực, tránh unmount ngoài viewport. Thiết lập `Page.setDownloadBehavior` (CDP) trỏ về `outDir` để Chrome tải trực tiếp file về folder gốc chuẩn, dọn sạch hoàn toàn tàn dư file ID ảo rác không có đuôi trong Chrome download history. Thiết lập `CONCURRENCY = 1` (tuần tự) cho đợt tải thứ hai (của cả upscale lần 1 lẫn tải lần 2) để tránh tranh chấp ghi file tạm playwright-artifacts trên Windows. |
| Không tìm thấy video element của clip vừa gen xong | UI Flow cần khoảng 2-5 giây để khởi tạo phần tử HTML và resolve thuộc tính `src` (chứa `/fx/`, `blob:`, `http` hoặc `googleusercontent`) sau khi render xong. Nếu quét ngay sẽ lỗi `No video element found`. Giải pháp: Chờ 5s cho DOM mount, tìm cả thẻ `video, img` trực tiếp trong link edit `a[href*="/edit/${editId}"]` thay vì chỉ tìm thẻ video thô, và check `src` hợp lệ trước khi click. | Đồng thời, video mới luôn nằm ở đầu grid (trên cùng), nếu cuộn trôi xuống đáy, virtual list sẽ unmount video đầu trang → Tuyệt đối KHÔNG cuộn xuống dưới để tìm video mới. |
| 20 | Video hiển thị = 0 / Không tìm thấy phần tử video sau khi render | Do tab "Xem video" chưa được chọn/chưa mount DOM. Tìm tab chứa text "Xem video" và click programmatically để các thẻ video mount | Tải lại trang hoặc chuyển tab thủ công |
...
- **Env vars (portability):**
  ...
  - `FLOW_RESOLUTION` — Độ phân giải tải về (default `1080p`, hỗ trợ: `720p`, `1080p`, `4k`). Cực kỳ quan trọng: CLI sẽ tự click chuột phải vào video/img trên Flow tại tọa độ trung tâm, hover mở submenu Radix UI và bấm chọn độ phân giải tương ứng. Việc phân tách upscale/tải dựa trên việc bắt request mạng `batchAsyncGenerateVideoUpsampleVideo` thời gian thực và tự động xếp hàng đợi chờ 90 giây. Tải chất lượng cao tự động chuyển sang cấu hình `CONCURRENCY = 1` ở lượt 2 để tránh xung đột ghi file tạm trên Windows.
...
| `flow-download.js` | `node flow-download.js <projectId> <outDir> [maxCount] [idsFile]` | Gom video (cuộn-gom) tải song song **8 luồng** (`CONCURRENCY=8`) + retry + `manifest.json`. `idsFile` = JSON array editId. **RESUME (Mới 2026-07-18):** Đọc trực tiếp từ `manifest.json` thay vì check regex tên file để tránh trùng lặp khi Chrome download file tạm (UUID không đuôi). |
...
- **Tải độ phân giải cao (1080p/4K) (Vá 2026-07-18):** Để tải video chất lượng cao (1080p hoặc 4K) thay vị trí mặc định 720p, bắt buộc phải dùng cơ chế Right-Click context menu của Flow. Quy trình tự động bằng Playwright:
  1. Đồng bộ hóa viewport ảo của CDP: BẮT BUỘC gọi `await page.setViewportSize({ width: 1920, height: 1080 })` ngay sau khi connect để tránh grid unmount các video ngoài màn hình ảo.
  2. Đưa video vào viewport: Đảm bảo scroll lên đỉnh `scrollTop = 0` trước khi tải và trước khi xử lý mỗi job trong worker loop vì video mới luôn nằm ở đầu grid. Tuyệt đối KHÔNG cuộn xuống dưới để tìm video mới trong worker loop.
  3. Chờ video mount: Đợi tối đa 5s cho DOM mount, và đợi `src` của video sẵn sàng (chứa `/fx/`, `blob:`, `http` hoặc `googleusercontent`) trước khi click.
  4. Click chuột phải bằng Playwright native click có `force: true` lên cả thẻ `video, img` (video mới render xong chỉ có thẻ `img` thumbnail, chưa có `video`) để bypass các button Radix UI che khuất.
  5. Đợi context menu Radix: `await page.locator('[role="menu"]').first().waitFor()`.
  6. Hover và Click mở rộng bằng evaluate click trực tiếp DOM node của mục "Tải xuống" để tránh lỗi Radix hover xịt: click item `aria-haspopup="menu"` chứa text "Tải xuống" bằng evaluate click.
  7. Đợi submenu Radix: `await page.locator('[role="menu"]').nth(1).waitFor()`.
  8. Click chọn độ phân giải và tự động phân tách trạng thái upscale: Lắng nghe song song sự kiện `download` của page và request mạng `batchAsyncGenerateVideoUpsampleVideo` thời gian thực bằng `Promise.race`:
     * **Nếu bắt được request mạng upsample**: Server vừa nhận lệnh upscale (chưa có file sẵn). Script tự động đóng toast/popup, đẩy job vào mảng tạm `pendingUpscaleJobs`. Sau khi kích hoạt upscale hết lượt, script nghỉ **90 giây** cho server xử lý, rồi đẩy ngược lại hàng đợi để tải thực tế.
     * **Nếu bắt được sự kiện download**: Tải file trực tiếp và gọi `download.saveAs(file)` để ghi xuống đĩa cứng.
  9. Hạn chế ghi trùng file tạm trên Windows: Sử dụng `CONCURRENCY = 1` (tải tuần tự) cho đợt tải thứ hai (đợt tải upscale) để tránh tranh chấp file tạm Playwright (`saveAs` ném lỗi ENOENT).
  10. Đóng context menu triệt để: Gửi `page.mouse.click(10, 10)` ra vùng trống và gửi phím `Escape` sau khi xử lý mỗi job để dọn sạch menu.
  11. Đối chiếu file đã tải bằng cách đọc trực tiếp từ `manifest.json` UUID `editId` thay vì check regex tên file cứng (do tên file download 1080p khác biệt).
  12. **Bypass tàn dư file ID rác không có đuôi trong Chrome History (Vá 2026-07-18):** Khi Playwright bắt download event bằng `page.waitForEvent('download')`, nó sẽ tải file tạm không đuôi về thư mục Temp của OS, sau đó `saveAs` di chuyển sang thư mục đích. Việc này để lại tàn dư file ID ảo không có đuôi trong Chrome Download Manager (khiến người dùng click vào báo file không tồn tại). Giải pháp triệt để:
      - Loại bỏ hoàn toàn sự kiện lắng nghe download của Playwright.
      - Sử dụng CDP ` cdpsession.send('Page.setDownloadBehavior', { behavior: 'allow', downloadPath: ... })` trỏ thẳng về `outDir`.
      - Khi click download, Chrome sẽ tự động lưu file có đuôi `.mp4` / `.crdownload` (chuẩn) trực tiếp vào `outDir`.
      - Quét thư mục `outDir` trong 30s để bắt file mới xuất hiện, đợi file mất đuôi `.crdownload` (hoàn tất tải), và đổi tên sang `video_01_*.mp4` chuẩn của job. Cách này giúp lịch sử tải của Chrome luôn hiển thị đúng file, đúng đuôi `.mp4` thật, click mở được ngay.
